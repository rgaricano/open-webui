...
class PDFGenerator:
    ...
    def _build_html_message(self, message: Dict[str, Any]) -> str:
        """Build MD for a single message."""
        role = markdown(message.get("role", "user"))
        content = markdown(message.get("content", ""))
        timestamp = message.get("timestamp")
        model = markdown(message.get("model") if role == "assistant" else "")
        date_str = markdown(self.format_timestamp(timestamp) if timestamp else "")
...
___"""
    ...
    def _generate_html_body(self) -> str:
        """Generate the full MD body for the PDF."""
        escaped_title = markdown(self.form_data.title)
