...
WEBUI_SECRET_KEY="$WEBUI_SECRET_KEY" exec uvicorn open_webui.main:app --host "$HOST" --port "$PORT" --forwarded-allow-ips '*' --ssl-certfile="/usr/local/IAI/open-webui/backend/selfsigned.crt" --ssl-keyfile="/usr/local/IAI/open-webui/backend/selfsigned.key"
